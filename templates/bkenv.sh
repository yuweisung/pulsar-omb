...
# Memory size options
BOOKIE_MEM=${BOOKIE_MEM:-${PULSAR_MEM:-"-Xms{{ (ansible_memtotal_mb*0.45)|int|string + 'm' }} -Xmx{{ (ansible_memtotal_mb*0.45)|int|string + 'm' }} -XX:MaxDirectMemorySize={{ (ansible_memtotal_mb*0.45)|int|string + 'm' }}"}}
